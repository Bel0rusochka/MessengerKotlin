...
class ServerMessageModel(override val dbName: String):DbModel<DbMessage> {
    ...
    override fun insertItem(item: DbMessage) {
    ...
    fun getAllClientItems(dstClientName: String): List<DbMessage> {
        ...
        try{
            ...
            while (tableDates?.next() == true) {
                val item = DbMessage(tableDates.getTimestamp("timestamp"),tableDates.getString("text"), tableDates.getString("dstClientName"), tableDates.getString("srcClientName"))
                items.add(item)
            }
        } catch (e: SQLException) {
            println(e.message)
...
data class DbMessage(val timestamp: Timestamp, val text: String, val dstClientName: String, val srcClientName: String)